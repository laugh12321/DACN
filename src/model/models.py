...
# ############################ attention ############################
def attention_pixel_based_cnn(n_classes: int, input_size: int, **kwargs) -> tf.keras.Model:
    """
...
def attention_cube_based_cnn(n_classes: int, input_size: int,
    ...
    pam = Position_attention(filters=16)(conv1)
    pam = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 5),
                                 padding='same',
                                 use_bias=False,
                                 kernel_initializer='he_normal',
                                 activation='relu')(pam)
    pam = tf.keras.layers.Dropout(0.2)(pam)
    pam = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 1),
                                 padding='same',
                                 use_bias=False,
                                 kernel_initializer='he_normal',
                                 activation='relu')(pam)
    ...
    cam = Channel_attention()(conv1)
    cam = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 5),
                                 padding='same',
                                 use_bias=False,
                                 kernel_initializer='he_normal',
                                 activation='relu')(cam)
    cam = tf.keras.layers.Dropout(0.2)(cam)
    cam = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 1),
                                 padding='same',
                                 use_bias=False,
                                 kernel_initializer='he_normal',
                                 activation='relu')(cam)
    ...
    feature_sum = tf.keras.layers.add([pam, cam])
    feature_sum = tf.keras.layers.Dropout(0.2)(feature_sum)
    feature_sum = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 1),
                                         padding='same',
                                         use_bias=False,
                                         kernel_initializer='he_normal',
                                         activation='relu')(feature_sum)
    ...
    conv3_d = tf.keras.layers.Dropout(rate=0.2)(conv3)
    flatten = tf.keras.layers.Flatten()(conv3_d)
    dense_1 = tf.keras.layers.Dense(units=192, activation='relu')(flatten)
    dense_2 = tf.keras.layers.Dense(units=150, activation='relu')(dense_1)
    dense_3 = tf.keras.layers.Dense(units=n_classes, activation='softmax')(dense_2)
...
def attention_pixel_based_dcae(n_classes: int, input_size: int,
    ...
    """
    input = tf.keras.layers.Input(shape=(1, 1, input_size, 1))
    conv1 = tf.keras.layers.Conv3D(filters=2, kernel_size=(1, 1, 3), activation='relu')(input)
    conv1_m = tf.keras.layers.MaxPool3D(pool_size=(1, 1, 2))(conv1)

    conv2 = tf.keras.layers.Conv3D(filters=4, kernel_size=(1, 1, 3), activation='relu')(conv1_m)
    conv2_m = tf.keras.layers.MaxPool3D(pool_size=(1, 1, 2))(conv2)

    conv3 = tf.keras.layers.Conv3D(filters=8, kernel_size=(1, 1, 3), activation='relu')(conv2_m)
    conv3_m = tf.keras.layers.MaxPool3D(pool_size=(1, 1, 2))(conv3)

    conv4 = tf.keras.layers.Conv3D(filters=16, kernel_size=(1, 1, 3), activation='relu')(conv3_m)
    conv3_m = tf.keras.layers.MaxPool3D(pool_size=(1, 1, 2))(conv4)

    conv5 = tf.keras.layers.Conv3D(filters=32, kernel_size=(1, 1, 3), activation='relu')(conv4)

    flatten = tf.keras.layers.Flatten()(conv5)
    dense_1 = tf.keras.layers.Dense(units=256, activation='relu')(flatten)
    dense_2 = tf.keras.layers.Dense(units=n_classes, activation='relu')(dense_1)
    dense_2 = tf.keras.layers.Softmax()(dense_2)
    # Decoder part (later to be dropped):
    dense_3 = tf.keras.layers.Dense(units=input_size, activation='relu',
                              weights=(np.swapaxes(kwargs['endmembers'], 1, 0), np.zeros(input_size)),
